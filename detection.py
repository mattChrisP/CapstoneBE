...
class ObjectDetection:
    ...
    def detect(self, img_path, idx, camId):
        ...
        for path, img, im0s, vid_cap in dataset:
            ...
            # Process detections
            cell_phone_centers += self.process_detection(pred, path, im0s, dataset, names, img, idx, camId)
        print(f'Done. ({time.time() - t0:.3f}s)')
        return cell_phone_centers

    def process_detection(self, pred, path, im0s, dataset, names, img, idx, camId):
        cell_phone_centers = []
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if int(cls) == 67:  # index for "cell phone"
                        ...
                        cv2.imwrite(f"out{idx}CAM{camId}.jpg",im0)
